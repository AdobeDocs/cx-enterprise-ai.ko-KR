---
title: Adobe Analytics에서 Customer Journey Analytics 데이터로의 유효성 검사
...
1. 설정 프로세스의 각 질문에 대해 답변을 선택한 다음 [!UICONTROL **제출**]&#x200B;을 선택합니다.

   설정 프로세스에는 아래 표의 질문이 포함되어 있습니다.
...
1. 계속하기 전에 설정 요약을 검토하여 올바른 데이터의 유효성을 검사하고 있는지 확인하십시오.

   요약에는 각 시스템의 상위 지표 및 차원에 대한 미리 보기와 함께 선택한 회사, 보고서 세트 및 데이터 보기가 포함됩니다.
...
1. [!UICONTROL **전체 일치율**] 탭을 선택하여 Adobe Analytics 보고서 세트의 데이터가 Customer Journey Analytics 데이터 보기의 데이터와 얼마나 가깝게 일치하는지 나타내는 백분율을 확인합니다.

   이 점수는 항상 다른 결과보다 먼저 나타납니다. 페이지 보기 수와 같은 대량 지표가 점수를 왜곡하지 않도록 비교되는 모든 지표와 차원의 가중치를 동일하게 부여합니다.
...
1. 분석 결과 하나를 한 문장으로 요약한 짧은 콜아웃 상자를 2~4개 보려면 [!UICONTROL **주요 인사이트**] 탭을 선택하십시오.

   설명선은 심각도별로 색상 코딩되므로 가장 중요한 결과를 먼저 식별할 수 있습니다.

1. 다음 정보를 보려면 [!UICONTROL **요약**] 탭을 선택하십시오.

   * Adobe Analytics 합계

   * Customer Journey Analytics 합계

   * 총 변량

   * 일 경과

     아래 설명된 [!UICONTROL **합격**] 분산 상태에 속하는 날짜 범위의 일 수를 반영합니다.

   * 중요 일수

     아래 설명된 [!UICONTROL **중요**] 분산 상태에 속하는 날짜 범위의 일 수를 반영합니다.

1. (조건부) 단일 차원 비교 또는 단일 지표 비교를 수행할 때 [!UICONTROL **일별 트렌드**] 탭을 선택하여 Adobe Analytics 데이터와 Customer Journey Analytics 데이터의 병렬 비교를 확인합니다.
...
1. (조건부) 단일 차원 비교 또는 단일 지표 비교를 수행할 때 [!UICONTROL **날짜 세부 정보**] 탭을 선택하여 비교된 각 지표 또는 차원 값에 대한 다음 정보를 확인하십시오.

   * 일자

   * Adobe Analytics 값

   * Customer Journey Analytics 값
...
1. (조건부) 전체 보고서 세트 및 데이터 보기 감사를 실행할 때 [!UICONTROL **스코어카드**] 탭을 선택하여 다음 정보를 확인합니다.
   ...
   * 상위 5개의 가장 일치하는 지표 및 상위 5개의 가장 일치하는 지표 및 차원을 나열하는 표

1. 추가 패턴 및 분석 중에 발견된 문제, 이러한 패턴에 대한 가능한 원인 및 데이터 불일치를 해결하기 위한 제안된 작업을 보려면 분석에서 아래로 스크롤하십시오.
...
1. (선택 사항) [확인할 데이터 선택](#choose-the-data-to-validate)에 설명된 대로 다른 지표를 분석하거나 다른 차원을 분석하거나 최대 40개 지표 및 20개 차원으로 구성된 다른 보고서를 실행하여 분석을 계속합니다.

   회사, 보고서 세트 및 데이터 보기 선택 사항이 대화 전체에서 수행되도록 설정 프로세스를 반복할 필요는 없습니다.

1. Customer Journey Analytics 업그레이드 가이드의 [권장 업그레이드 단계](https://experienceleague.adobe.com/ko/docs/analytics-platform/using/compare-aa-cja/upgrade-to-cja/cja-upgrade-recommendations#recommended-upgrade-steps-for-most-organizations) 또는 동적으로 생성된 업그레이드 단계를 계속 수행합니다.

   Customer Journey Analytics에서 Customer Journey Analytics 업그레이드 가이드에 액세스하려면 **[!UICONTROL Workspace]** 탭을 선택한 다음 왼쪽 패널에서 **[!UICONTROL Customer Journey Analytics으로 업그레이드]**&#x200B;를 선택합니다. 화면에 표시되는 안내를 따릅니다.
